...
from app.models.stores import StoreReviewRead
...
from app.services.stores import StoreReviewsService
from ..util import get_exception_docs

router = APIRouter(prefix="/stores/{store_id}/reviews", tags=["Store reviews"])
...
async def create_store_review(
    store_id: Id,
    review: ReviewCreate,
    reviews_service: StoreReviewsService = Depends(),
    caller_id: Id = Depends(get_caller_id),
) -> StoreReviewRead:
    return await reviews_service.create_review(review, store_id, caller_id)
...
async def get_store_reviews(
    ...
    reviews_service: StoreReviewsService = Depends(),
) -> ReviewList[StoreReviewRead]:
    reviews = await reviews_service.get_reviews(
        limit, offset, sort_by, sort_order, store_id=store_id
    )
    count, average = await reviews_service.count_and_average_reviews(store_id=store_id)
...
async def get_my_store_review(
    store_id: Id,
    reviews_service: StoreReviewsService = Depends(),
    caller_id: Id = Depends(get_caller_id),
) -> StoreReviewRead:
    return await reviews_service.get_review_by_id(store_id, caller_id)


@router.get("/{user_id}", responses=get_exception_docs(REVIEW_NOT_FOUND_ERROR))
async def get_user_store_review(
    store_id: Id,
    user_id: Id,
    reviews_service: StoreReviewsService = Depends(),
) -> StoreReviewRead:
    return await reviews_service.get_review_by_id(store_id, user_id)


@router.put("/me", responses=get_exception_docs(REVIEW_NOT_FOUND_ERROR))
async def update_my_store_review(
    store_id: Id,
    review: ReviewCreate,
    reviews_service: StoreReviewsService = Depends(),
    caller_id: Id = Depends(get_caller_id),
) -> StoreReviewRead:
    return await reviews_service.update_review(review, store_id, caller_id)
...
async def delete_my_store_review(
    store_id: Id,
    reviews_service: StoreReviewsService = Depends(),
    caller_id: Id = Depends(get_caller_id),
) -> None:
    await reviews_service.delete_review(store_id, caller_id)
